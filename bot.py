...
def start(message):
    ...
    bot.send_photo(message.chat.id, photo,
                   caption="Добро пожаловать в Place2Day. Для удостоверения что вы не робот, получите код верификации:",
                   reply_markup=markup)
...
def func(message):
    if message.text == "Подтвердить аккаунт":
        ...
        bot.send_message(message.chat.id,
                         text="Вы уверены? Действие невозможно отменить. Чтобы вернуться назад нажмите /start",
                         reply_markup=markup)
        ...
        try:
            ...
            try:
                ...
                if only(str(message.chat.id)) == False:
                    ...
                    bot.send_photo(message.chat.id, photo,
                                   caption="К телеграму уже привязан аккаунт. Он пройдет проверку администрации на нарушения.")
                else:
